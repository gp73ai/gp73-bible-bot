...
//
// In addition to the base product fields, this endpoint now also resolves:
//   - offer_id: the id of the active offer for this product (or null)
//   - has_checkout: whether an active offer AND a mapped checkout
//     destination both exist for this product. The storefront uses this
//     (not just `status`) to decide whether to show a real Purchase button
//     or a disabled "Coming Soon" button, per the Creator Commerce
//     checkout-routing requirements.
//
// This does not add or alter any Supabase table/column — it only reads the
// existing `offers` and `checkout_destinations` tables that already exist.
...
export default async function handler(req, res) {
  ...
  try {
        const productUrl =
                `${SUPABASE_URL}/rest/v1/products?select=id,brand_slug,slug,category,product_type,display_name,short_description,long_description,benefits,cover_image,preview_asset,status,offers(id,price_usd,price_label,offer_type,active)` +
      ...
      const product = {
              ...
              status: row.status,
              offer_id: offer && offer.active ? offer.id : null,
              has_checkout: offer && offer.active ? hasCheckout : false,
      };
